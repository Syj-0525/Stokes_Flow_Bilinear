...
for ee = 1: n_el   % 遍历所有单元
    for aa = 1: 4  % 遍历单元结点
        eleCtrlPts(aa,:) = CtrlPts(IEN(ee, aa), :);   % 第i个速度单元的结点坐标数据
    end
    ...
    for aa = 1:4
        for bb = 1:4
            C1(IEN(ee, aa), IEN(ee, bb)) = C1(IEN(ee, aa), IEN(ee, bb)) + Ce1(aa, bb);
            C2(IEN(ee, aa), IEN(ee, bb)) = C2(IEN(ee, aa), IEN(ee, bb)) + Ce2(aa, bb);
            A11(IEN(ee, aa), IEN(ee, bb)) = A11(IEN(ee, aa), IEN(ee, bb)) + Ae11(aa, bb);
            A12(IEN(ee, aa), IEN(ee, bb)) = A12(IEN(ee, aa), IEN(ee, bb)) + Ae12(aa, bb);
            A21(IEN(ee, aa), IEN(ee, bb)) = A21(IEN(ee, aa), IEN(ee, bb)) + Ae21(aa, bb);
            A22(IEN(ee, aa), IEN(ee, bb)) = A22(IEN(ee, aa), IEN(ee, bb)) + Ae22(aa, bb);  
            B1(IEN(ee, aa), IEN(ee, bb)) = B1(IEN(ee, aa), IEN(ee, bb)) + Be1(aa, bb);
            B2(IEN(ee, aa), IEN(ee, bb)) = B2(IEN(ee, aa), IEN(ee, bb)) + Be2(aa, bb);  
        end
    end
    %%%%%% 装配B1, B2, A11, A12, A21, A22, C1, C2
end
